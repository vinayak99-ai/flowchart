...
CLASSIFY_SYSTEM_PROMPT = """You are an infographic designer. Given source material and a user \
prompt, decide which fixed-layout infographic template best fits the content, before anything \
is generated.
...
are parts of one whole, not alternatives a reader has to weigh against each other.
...
naturally structured as parallel alternatives.

Pick the single best-fitting template. If the material doesn't clearly fit either, prefer \
whichever one loses less information.
"""
...
) -> InfographicWheel | InfographicComparison:
