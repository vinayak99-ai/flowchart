...
export function Rail({ activeTool, onSelect }: RailProps) {
  return (
    <aside className="flex w-16 shrink-0 flex-col items-center gap-1.5 border-r border-neutral-200 bg-white py-4">
        ...
        return (
          <div key={tool.id} className="group relative">
            ...
            >
              ...
              {tool.status === 'soon' ? (
                <span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full border border-white bg-accent" />
              ) : null}
            </button>
