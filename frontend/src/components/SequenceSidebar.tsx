...
import { extractFile, openSequenceGenerateSocket } from '../lib/api'
import type { GenerateSequenceResponse, SequenceWsProgressMessage } from '../sequenceTypes'
...
interface SequenceSidebarProps {
  onResult: (result: GenerateSequenceResponse) => void
...
const STATUS_LABEL: Record<Status, string> = {
  ...
  done: 'Sequence diagram ready.',
  error: 'Something went wrong.',
}

export function SequenceSidebar({ onResult, issues }: SequenceSidebarProps) {
  ...
  const handleGenerate = useCallback(() => {
    ...
    const onMessage = (message: SequenceWsProgressMessage) => {
    ...
    closeSocketRef.current = openSequenceGenerateSocket(material, prompt, onMessage, onError)
    ...
    <aside className="flex h-full w-[360px] shrink-0 flex-col gap-4 overflow-y-auto border-r border-neutral-200 bg-white p-4">
      <div>
        ...
        <textarea
          ...
          placeholder="Paste source material (an API spec, incident writeup, transcript, etc.)"
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="e.g. Map the login flow as a sequence diagram between client, server, and database."
      ...
      >
        {isGenerating ? 'Generating…' : 'Generate sequence diagram'}
      </button>
