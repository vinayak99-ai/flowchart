...
export function WheelPreview({ wheel }: WheelPreviewProps) {
  const items = wheel.items.slice(0, 5)
  while (items.length < 5) items.push({ label: '', description: '' })
      ...
      <g fontFamily="inherit">
        {items.map((item, i) => (
          <g key={i} transform={`translate(432,${58 + i * 104})`}>
            ...
            <text x="60" y="24" fontSize="13.5" fontWeight="800" fill="#fff">
              {item.label.toUpperCase()}
            </text>
            <foreignObject x="60" y="31" width="88" height="54">
              <div style={{ fontSize: '10px', lineHeight: 1.25, color: '#fff7ec', opacity: 0.92 }}>
                {item.description}
              </div>
            </foreignObject>
          </g>
