...
One process, one `.env` file, serving both Flowchart Builder's own routes and Spec
Builder's (mounted at `/pm`):
...
|---|---|
| `OPENAI_API_KEY` | Flowchart Builder's OpenAI API key (required) |
...
| `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` | Spec Builder's key, matching whichever provider `AIPM_MODEL` names — independent of Flowchart Builder's `OPENAI_API_KEY` above even when both happen to point at OpenAI, since they're separate provider configs |
